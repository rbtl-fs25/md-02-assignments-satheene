...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    geom_col() uses stat_identity() and thus leaves the data unchanged (use if you want the height of the bars to represent the data), while gem_bar() uses stat_count(), which counts the amounts of cases for the value x (use if you want the height of the bar to respresent the number of cases). In this case they are both the same since the number of cases and the value is the same in terms of count. They are thus in this case basically doing the same.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    Depending on which of the three values we change in seq() we observe that the first value tells us where the plot starts (x-value), while the second value tells us until where the values are plotted (x-value). The third value gives us the thickness of the bars in the historgram
...
world <- ne_countries(scale = "small", returnclass = "sf") #Why do I have to run this one seperately in the console before it works? is this normal?
